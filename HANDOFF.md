...
当前阶段：**第七轮（进行中）：站位归地**——IMU 视角跟随（水平线对齐）＋脚下阴影＋「AR站立视角」快捷键；上一轮（第六轮）已完成：AR 透明相机全链路＋眼神追踪 LookAt 初版。
...
2. ~~眼神追踪 LookAt 摄像头~~ ✅ 初版已完成（第六轮）：眼睛主跟随镜头＋头部轻跟随（±18°）；参数 `LOOKAT_EYE_GAIN` / `LOOKAT_HEAD_LIMIT`；待实机验证方向与自然度
3. ~~Spring Bone 物理~~ ✅ 用户确认头发/裙摆弹簧物理已生效（勿动）
